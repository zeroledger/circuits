...
contract PlonkVerifier {
    ...
    uint16 constant nPublic   = 14;
    uint16 constant nLagrange = 14;
    
    uint256 constant Qmx  = 15679323749354991543815716887949761140879633189784735641326614731469992353705;
    uint256 constant Qmy  = 12307102419039146426756932482236652977592550029459090713480248852831849435840;
    uint256 constant Qlx  = 3703350688248861523031273630164356362284971816911731255552072035592616884102;
    uint256 constant Qly  = 5602295940341967201710628593982414291101388098589420571909998517733174999877;
    uint256 constant Qrx  = 8625510299529168068977363147776096550776697362219049426168454138016928584319;
    uint256 constant Qry  = 21585767620068024487429907962797994559795469472125308946353526905687777067656;
    uint256 constant Qox  = 10530603637097035395400418542230995749416699199903626730810658496017459731769;
    uint256 constant Qoy  = 18161643837869420285633422321993242706064178815812425506676287476639018540409;
    uint256 constant Qcx  = 7523932982678064519280332005227272256065573710348069779625409048018153839438;
    uint256 constant Qcy  = 4392235539297027973341400029130392290338970507802866951316818802358735869649;
    uint256 constant S1x  = 8763384650665808150568493376031636803154289459580705662367405620301859705135;
    uint256 constant S1y  = 16854668166594424706836940306088837174346586327963286431208117686135745958911;
    uint256 constant S2x  = 16077064528629888059138630688061530394323051104180590550247453780485615397376;
    uint256 constant S2y  = 7253996236954942530858421081509831911256568778298311155922109337390162157397;
    uint256 constant S3x  = 8418383490540675074599588399932059285132289269546643861115788395069188166550;
    uint256 constant S3y  = 16558893522859178088351253045103268115698817034188505943286151255456109131488;
    ...
    uint16 constant lastMem = 1248;

    function verifyProof(uint256[24] calldata _proof, uint256[14] calldata _pubSignals) public view returns (bool) {
        assembly {
            /////////
            ...
            function calculateChallenges(pMem, pPublic) {
                ...
                mstore(add(mIn, 960 ), calldataload(pA))
                mstore(add(mIn, 992 ), calldataload(add(pA, 32)))
                mstore(add(mIn, 1024 ), calldataload(pB))
                mstore(add(mIn, 1056 ), calldataload(add(pB, 32)))
                mstore(add(mIn, 1088 ), calldataload(pC))
                mstore(add(mIn, 1120 ), calldataload(add(pC, 32)))
                
                beta := mod(keccak256(mIn, 1152), q) 
            ...
            function calculateLagrange(pMem) {
                ...
                inverseArray(add(pMem, pZhInv), 15 )
