...
contract PlonkVerifier {
    ...
    uint16 constant nPublic   = 18;
    uint16 constant nLagrange = 18;
    
    uint256 constant Qmx  = 20639010535096123574638369086843805343136745321006144501829672965141471869609;
    uint256 constant Qmy  = 6334938303669704817230403699736272558275137597841080157514946617070472574847;
    uint256 constant Qlx  = 7426894814857450604727467148758999258900482288419504650856551452457712007512;
    uint256 constant Qly  = 11845915359132697802847703433228958824103994375089304080464771452293932588190;
    uint256 constant Qrx  = 7042747478256434696964031641895823906348788685610173913029799301469422011329;
    uint256 constant Qry  = 19849273080475335106077299004904711312783596437770137679456647652402088040202;
    uint256 constant Qox  = 3017957647919599950370080009064975321869994801011363032984645264194832435450;
    uint256 constant Qoy  = 16626067492450524827862134209947652493190914767456469612979220756563161086625;
    uint256 constant Qcx  = 13001408400509587097060403446984508892559564292774028129237299171796952150574;
    uint256 constant Qcy  = 5836232534632298164971212782037811491024553479136383059609756787627402686541;
    uint256 constant S1x  = 17900036452922275861805236980268409036287994319716007751247391744345625924030;
    uint256 constant S1y  = 20245855170772313519270088701207634679176510156184240376723517989722703456413;
    uint256 constant S2x  = 19491176425599609751980583612922548814054779529522543062800829043629012312710;
    uint256 constant S2y  = 3565049609926862615009188895421494612645916753713174401641096704328177914841;
    uint256 constant S3x  = 14929623003703230316887371885483515882241682154823545709850301578964570742141;
    uint256 constant S3y  = 10195355178818405737326084074566746231452295449638186687955322966858077308582;
    ...
    uint16 constant lastMem = 1376;

    function verifyProof(uint256[24] calldata _proof, uint256[18] calldata _pubSignals) public view returns (bool) {
        assembly {
            /////////
            ...
            function calculateChallenges(pMem, pPublic) {
                ...
                mstore(add(mIn, 1088 ), calldataload(pA))
                mstore(add(mIn, 1120 ), calldataload(add(pA, 32)))
                mstore(add(mIn, 1152 ), calldataload(pB))
                mstore(add(mIn, 1184 ), calldataload(add(pB, 32)))
                mstore(add(mIn, 1216 ), calldataload(pC))
                mstore(add(mIn, 1248 ), calldataload(add(pC, 32)))
                
                beta := mod(keccak256(mIn, 1280), q) 
            ...
            function calculateLagrange(pMem) {
                ...
                inverseArray(add(pMem, pZhInv), 19 )
