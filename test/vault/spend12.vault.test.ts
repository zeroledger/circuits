...
describe("Vault Spend12 Tests", function () {
    describe("Spend12 Functionality", function () {
        describe("Successful Spends", function () {
            it("should successfully spend 1 input into 2 outputs with valid ZK proof", async function () {
                const { vault, mockToken, user, feeRecipient } =
                ...
                // Balances should change due to fee transfer from vault to fee recipient
                expect(finalBalances.user).to.equal(initialBalances.user);
                expect(finalBalances.vault).to.equal(
                    initialBalances.vault - testData.fee
                );
