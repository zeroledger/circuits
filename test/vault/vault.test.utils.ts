...
import { DepositVerifier } from "../../typechain-types/contracts/DepositVerifier";
import {
    TransactionStruct,
    OutputsOwnersStruct,
} from "../../typechain-types/contracts/Vault";
...
export async function deployVaultFixture(): Promise<TestFixture> {
    ...
    const Spend161VerifierFactory = await ethers.getContractFactory(
        "Spend161Verifier"
    );
    const spend161Verifier = await Spend161VerifierFactory.deploy();
    await spend161Verifier.waitForDeployment();

    const EncryptionRegistryFactory = await ethers.getContractFactory(
        "EncryptionRegistry"
    );
    const encryptionRegistry = await EncryptionRegistryFactory.deploy();
    await encryptionRegistry.waitForDeployment();
    ...
    const vault = await VaultFactory.deploy(
        ...
        await encryptionRegistry.getAddress()
    );
...
export async function generateDepositProof(
    ...
    const { calldata_proof } = await exportSolidityCallData(
        proof,
        publicSignals
    );
...
export function verifyDepositBalances(
    ...
    expect(finalBalances.vault).to.equal(
        initialBalances.vault + testData.depositAmount
    );
    expect(finalBalances.feeRecipient).to.equal(
        initialBalances.feeRecipient + testData.fee
    );
}
...
export async function verifyCommitments(
    ...
    for (let i = 0; i < hashes.length; i++) {
        const commitment = await vault.commitmentsMap(
...
export function verifyDepositEvents(receipt: any) {
    ...
    const tokenDepositedEvent = receipt?.logs?.find(
        (log: any) => log.fragment?.name === "TokenDeposited"
    );
    expect(tokenDepositedEvent).to.not.be.undefined;

    // Check for CommitmentCreated events
    const commitmentCreatedEvents = receipt?.logs?.filter(
        (log: any) => log.fragment?.name === "CommitmentCreated"
    );
    expect(commitmentCreatedEvents).to.have.length(3);

    const encryptedMetadataEvents = receipt?.logs?.filter(
        (log: any) => log.fragment?.name === "EncryptedMetadata"
    );
    expect(encryptedMetadataEvents).to.have.length(3);
}
...
export function verifyWithdrawBalances(
    ...
    expect(finalBalances.vault).to.equal(
        initialBalances.vault - testData.amount
    );
}

// Helper function to verify commitment was removed
export async function verifyCommitmentRemoved(
    hash: string,
    vault: Vault,
    mockToken: MockERC20
) {
...
export function verifyWithdrawEvents(receipt: any) {
    ...
    const commitmentRemovedEvent = receipt?.logs?.find(
        (log: any) => log.fragment?.name === "CommitmentRemoved"
    );
...
export function verifySpendEvents(
    receipt: any,
    inputHashes: string[],
    outputHashes: string[]
) {
    ...
    const transactionSpentEvent = receipt?.logs?.find(
        (log: any) => log.fragment?.name === "TransactionSpent"
    );
    expect(transactionSpentEvent).to.not.be.undefined;

    const commitmentRemovedEvents = receipt?.logs?.filter(
        (log: any) => log.fragment?.name === "CommitmentRemoved"
    );
    ...
    expect(encryptedMetadataEvents?.length ?? 0).to.equal(outputHashes.length);
}

// Helper function to verify input commitments were removed
export async function verifyInputCommitmentsRemoved(
    inputHashes: string[],
    vault: Vault,
    mockToken: MockERC20
) {
